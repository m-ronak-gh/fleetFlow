    ...
    return (
        <div className="min-h-screen bg-[#060010] p-4 md:p-8 font-sans relative overflow-hidden flex flex-col md:flex-row gap-6">
            ...
            <aside className={`relative z-30 transition-all duration-700 ease-in-out ${isNavOpen ? 'md:w-64 w-full h-auto' : 'md:w-24 w-full h-24'} bg-white/5 backdrop-blur-3xl rounded-[2.5rem] border border-white/10 flex flex-col items-center shadow-2xl overflow-hidden`}>
                ...
                <div className={`w-full px-4 space-y-2 mb-8 flex-1 overflow-y-auto no-scrollbar transition-all duration-700 ${isNavOpen ? 'opacity-100 flex flex-col' : 'opacity-0 md:opacity-100 flex flex-col'}`}>
                    {navItems.map((item) => (
                        <button
                            key={item.id}
                            onClick={() => handleNavClick(item.id)}
                            className={`w-full flex items-center ${isNavOpen ? 'justify-start px-6 gap-4' : 'justify-center'} p-4 rounded-[1.5rem] transition-all group shrink-0 ${activeTab === item.id ? 'text-white bg-primary shadow-lg shadow-primary/20 scale-[1.02]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
                        >
                            <item.icon size={24} className={`${activeTab === item.id ? 'text-white' : 'group-hover:text-primary transition-colors'}`} />
                            {isNavOpen && <span className="font-bold text-xs tracking-widest uppercase text-left whitespace-nowrap animate-in fade-in slide-in-from-left-2 duration-300">{item.label}</span>}
                        </button>
                    ))}
                </div>
            ...
            {/* Main Content Area */}
            <main className="flex-1 transition-all duration-700 relative z-10 space-y-6 overflow-y-auto no-scrollbar">
